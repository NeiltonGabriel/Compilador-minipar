...
EXAMPLES = {
    ...
    "▶ Teste 1: Calculadora Cliente/Servidor": _load_example('teste1.minipar'),
    "▶ Teste 2: Execução paralela (PAR) - Fatorial e Fibonacci": _load_example('teste2.minipar'),
    "▶ Teste 3: Neurônio": _load_example('teste3.minipar'),
    "▶ Teste 4: Rede Neural XOR": _load_example('teste4.minipar'),
    "▶ Teste 5: Sistema de recomendação": _load_example('teste5.minipar'),
    "▶ Teste 6: Quicksort Orientado a Objetos": _load_example('teste6.minipar'),
    "▶ Teste 7: Fatorial iterativo": _load_example('teste7.minipar'),
    "▶ Teste 8: Série de Fibonacci": _load_example('teste8.minipar'),
    "▶ Teste 9: Quicksort Iterativo": _load_example('teste9.minipar'),
    "▶ Teste 10: fractal": _load_example('teste10fractal.minipar'),
}
...
def action_compilar_gcc(source: str):
    """Compila para executável nativo e disponibiliza os artefatos para download.

    Retorna (texto_resultado, lista_de_arquivos) — o executável é mantido (não
    é mais apagado) junto do .c e do .s gerados, na pasta build/.
    """
    if not source.strip():
        return "⚠️  Nenhum código.", []
    try:
        ...
        c_path   = os.path.join(_BUILD_DIR, 'programa.c')
        s_path   = os.path.join(_BUILD_DIR, 'programa.s')
        exe_path = os.path.join(_BUILD_DIR, 'programa')

        # Salvar o .c (estável, para download)
        with open(c_path, 'w', encoding='utf-8') as f:
            f.write(c_src)
        # Salvar também o assembly ARM gerado
        try:
            s_src = ArmCodeGenerator(table).generate(tac)
        ...
        gcc = GCCBackend()
        result = gcc.compile_to_executable(c_path, exe_path, features)
        if result.success:
            arquivos = [p for p in (exe_path, c_path, s_path)
                        if p and os.path.exists(p)]
            texto = (
                f"✅  Compilação bem-sucedida!\n"
                f"    Executável: {exe_path}\n"
                f"    Features:   {features or {'básico'}}\n\n"
                f"    ⬇  Baixe o executável (e o .c/.s) na área abaixo.\n\n"
...
def build_interface() -> gr.Blocks:
    with gr.Blocks(
        title="MiniPar Compiler v2026.1",
...
def main():
    p = argparse.ArgumentParser(
        description="Interface gráfica do compilador MiniPar v2026.1"
    )
    ...
    app = build_interface()
    print(f"\n{'='*55}")
    print(f"  MiniPar Compiler v2026.1 — Interface Web")
    print(f"{'='*55}")

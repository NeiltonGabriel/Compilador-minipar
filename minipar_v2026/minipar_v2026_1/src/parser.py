"""
parser.py — MiniPar v2026.1
Parser descendente recursivo LL(1) estendido.
100% backward-compatible: todos os programas v2026.1 continuam parseando.
"""
...
class Parser:
    ...
    def declaracao(self):
        ...
        # ── Existentes v2026.1 (inalterados) ──────────────────────────────────
    ...
    def decl_classe(self, abstrata: bool = False) -> ClassDecl:
        ...
        while not self.casa(TokenType.RBRACE, TokenType.EOF):
            ...
            # Modificadores de visibilidade (aceitos; sem controle de acesso).
            if self.casa(TokenType.PUBLIC, TokenType.PRIVATE,
                         TokenType.PROTECTED):
                self.avancar()
            ...
            if self.casa(TokenType.OVERRIDE):
                is_override = True
                self.avancar()
            # Visibilidade também pode vir depois de static/override.
            if self.casa(TokenType.PUBLIC, TokenType.PRIVATE,
                         TokenType.PROTECTED):
                self.avancar()
            ...
            if self.casa(TokenType.VAR):
                ...
                # Construtor: func ClassName(...) { ... }
                if (self.atual().valor == nome
                        and self.atual().tipo == TokenType.IDENTIFIER
                        and self.peek().tipo == TokenType.LPAREN):
                    ...
                    # Método (sintaxe antiga 'tipo nome(...)' ou professor
                    # 'nome(...) -> tipo').
                    method_nome, tipo_ret, params = self._parse_cabecalho_funcao()
                    corpo = self.bloco()
    ...
    def decl_funcao_async(self) -> AsyncFuncDecl:
        ...
        nome, tipo_ret, params = self._parse_cabecalho_funcao()
        corpo = self.bloco()
    ...
    def decl_variavel(self) -> VarDecl:
        ...
        nome, tipo = self._parse_nome_tipo()
        inicializador = None
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Comandos existentes v2026.1 (inalterados)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _parse_expr_or_assign(self):
        ...
        if self.casa(TokenType.ASSIGN):
            ...
            # Atribuição por índice: lista[i] = valor
    ...
    def _primary(self):
        ...
        if self.casa(TokenType.IDENTIFIER, TokenType.S_CHANNEL,
                     TokenType.C_CHANNEL):
            nome = self.avancar().valor
            if self.casa(TokenType.LPAREN):
                # Chamada de função (inclui construtores de canal)
                self.avancar()
                args = self._parse_argumentos()
                self.consumir(TokenType.RPAREN, "Esperado ')'")
    ...
    def tipo_especificador(self) -> str:
        tipos_validos = {
            ...
            TokenType.S_CHANNEL, TokenType.C_CHANNEL,
        }
    ...
    def _parse_parametros(self) -> List[VarDecl]:
        params = []
        if self.casa(TokenType.RPAREN, TokenType.EOF):
            return params
    ...
    def decl_variavel_parametro(self) -> VarDecl:
        """Parâmetro nas duas sintaxes (sem inicializador, sem ;):
        - antiga:    var tipo nome
        - professor: nome: tipo   (ou  var nome: tipo)"""
        linha = self.atual().linha
        if self.casa(TokenType.VAR):
            self.avancar()                 # 'var' é opcional na sintaxe nova
        nome, tipo = self._parse_nome_tipo()
        return VarDecl(nome=nome, tipo=tipo, inicializador=None, linha=linha)

    # ── Helpers de sintaxe dupla (antiga vs. professor) ───────────────────────
    def _parse_nome_tipo(self):
        """Lê um par (nome, tipo) em qualquer ordem:
        - professor: nome ':' tipo
        - antiga:    tipo nome
        Retorna (nome, tipo)."""
        if (self.atual().tipo == TokenType.IDENTIFIER
                and self.peek().tipo == TokenType.COLON):
            nome = self.avancar().valor
            self.consumir(TokenType.COLON, "Esperado ':'")
        ...
        tipo = self.tipo_especificador()
        nome = self.consumir(
            TokenType.IDENTIFIER, "Esperado nome da variável"
        ).valor
        return nome, tipo

    def _parse_cabecalho_funcao(self):
        """Cabeçalho de função nas duas sintaxes, após 'func' já consumido:
        - professor: nome '(' params ')' '->' tipo
        - antiga:    tipo nome '(' params ')'
        Retorna (nome, tipo_retorno, params)."""
        if self.peek().tipo == TokenType.LPAREN:
            nome = self.consumir(
                TokenType.IDENTIFIER, "Esperado nome da função"
            ).valor
            self.consumir(TokenType.LPAREN, "Esperado '('")
            params = self._parse_parametros()
            self.consumir(TokenType.RPAREN, "Esperado ')'")
            tipo_ret = 'void'
            if self.casa(TokenType.ARROW):
                self.avancar()
                tipo_ret = self.tipo_especificador()
            return nome, tipo_ret, params
        tipo_ret = self.tipo_especificador()

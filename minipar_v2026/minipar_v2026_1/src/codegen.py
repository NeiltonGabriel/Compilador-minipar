...
class CodeGenerator:
    ...
    def __init__(self, symbol_table: SymbolTable):
        ...
        self._current_class: Optional[str] = None   # classe do método/ctor atual
        self._local_types: dict = {}   # nome local (param/temp/var) → nome de classe
        self._local_names: set = set()  # todos os nomes locais (params + var decls)

    # ── Resolução de acesso implícito a campo (sem 'this.') ────────────────────
    def _current_field(self, name):
        """Se 'name' for um campo da classe atual e NÃO um local/param, retorna
        o descritor do campo; caso contrário, None. Permite acesso implícito a
        campos (ex.: `cor` em vez de `this.cor`)."""
        if not self._current_class or not isinstance(name, str):
            return None
        if name in self._local_names or name == '__this__':
            return None
        desc = self.table.lookup_class(self._current_class)
        if not desc:
            return None
        return desc.lookup_field(name)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Declarações existentes v2026.1
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def gen_FuncDecl(self, node: FuncDecl):
        prev_names = self._local_names
        self._local_names = {p.nome for p in node.parametros}
        ...
        self._local_names = prev_names

    def gen_VarDecl(self, node: VarDecl):
        self._local_names.add(node.nome)
        if self.table.lookup_class(node.tipo):
            self._local_types[node.nome] = node.tipo
    ...
    def gen_AssignStmt(self, node: AssignStmt):
        ...
        if isinstance(node.alvo, str):
            # Atribuição a campo implícito da classe atual (sem 'this.')
            f = self._current_field(node.alvo)
            if f is not None:
                self.emit(TACOp.STORE_FIELD, '__this__', val, str(f.offset))
            else:
    ...
    def gen_FunctionCall(self, node: FunctionCall) -> str:
        arg_vals = [self.generate(arg) for arg in node.argumentos]
        for v in arg_vals:
    ...
    def gen_Identifier(self, node: Identifier) -> str:
        # Acesso implícito a campo da classe atual (sem 'this.')
        f = self._current_field(node.nome)
        if f is not None:
            result = self.new_temp()
            self.emit(TACOp.LOAD_FIELD, '__this__', str(f.offset), result)
            if self.table.lookup_class(f.tipo_dados):
                self._local_types[result] = f.tipo_dados
                self._type_map[result] = f.tipo_dados
            return result
        return node.nome
    ...
    def _gen_constructor(self, ctor: ConstructorDecl, desc: 'ClassDescriptor'):
        prev_class, prev_locals = self._current_class, self._local_types
        prev_names = self._local_names
        self._current_class = desc.nome
        self._local_types = {}
        self._local_names = {p.nome for p in ctor.parametros}
        for param in ctor.parametros:
            if self.table.lookup_class(param.tipo):
                self._local_types[param.nome] = param.tipo
        ...
        self._current_class, self._local_types = prev_class, prev_locals
        self._local_names = prev_names
    ...
    def _gen_method(self, metodo: MethodDecl, class_name: str):
        ...
        self._local_types = {}
        self._local_names = {p.nome for p in metodo.parametros}
    ...
    def gen_NewExpr(self, node: NewExpr) -> str:
        ...
        self._local_types[result] = node.class_name
        return result
    ...
    def gen_MemberAccessExpr(self, node: MemberAccessExpr) -> str:
        ...
        # propagar tipo se o campo for um objeto
        if self.table.lookup_class(f.tipo_dados):
            self._local_types[result] = f.tipo_dados
            self._type_map[result] = f.tipo_dados
        return result
    ...
    def gen_MethodCallExpr(self, node: MethodCallExpr) -> str:
        ...
        # Avaliar argumentos ANTES de emitir PARAM (evita interleaving com
        # chamadas aninhadas, que corromperia a sequencia de PARAM).
        arg_vals = [self.generate(arg) for arg in node.argumentos]
        ...
        if desc and desc.lookup_method(node.metodo):
            # Despacho dinâmico via VTable (todo método é virtual).
            # arg2 = "idx@Classe" para o backend resolver a assinatura.
            idx = desc.get_vtable_index(node.metodo)
            entry = desc.lookup_method(node.metodo)
            vtbl = self.new_temp()
            self.emit(TACOp.LOAD_VTABLE, ptr, None, vtbl)
            self.emit(TACOp.VCALL, vtbl, f'{idx}@{tipo_obj}', result)
            if self.table.lookup_class(entry.return_type):
                self._local_types[result] = entry.return_type
                self._type_map[result] = entry.return_type
        else:
            total = len(node.argumentos) + 1
            self.emit(TACOp.CALL, node.metodo, str(total), result)
    ...
    def gen_SuperCallExpr(self, node: SuperCallExpr) -> str:
        # Superclasse a partir do contexto de classe atual
        super_type = 'unknown'
        if self._current_class:
            desc = self.table.lookup_class(self._current_class)
            if desc and desc.superclasse:
                super_type = desc.superclasse.nome
        arg_vals = [self.generate(arg) for arg in node.argumentos]
    ...
    def _infer_type_name(self, node) -> str:
        """
        Tenta inferir o nome do tipo (classe) de um no de expressao.
        Usado para encontrar o ClassDescriptor correto.
        """
        if isinstance(node, ThisExpr):
            return self._current_class or 'unknown'
        if isinstance(node, NewExpr):
            return node.class_name
        if isinstance(node, Identifier):
            if node.nome in self._local_types:
                return self._local_types[node.nome]
            if self._current_class:
                desc = self.table.lookup_class(self._current_class)
                if desc:
                    f = desc.lookup_field(node.nome)
                    if f and self.table.lookup_class(f.tipo_dados):
                        return f.tipo_dados
            sym = self.table.lookup(node.nome)
            if sym:
                return sym.tipo_dados
        if isinstance(node, MemberAccessExpr):
            base = self._infer_type_name(node.obj)
            desc = self.table.lookup_class(base)
            if desc:
                f = desc.lookup_field(node.membro)
                if f:
                    return f.tipo_dados
        ...
        if isinstance(node, str) and node in self._type_map:

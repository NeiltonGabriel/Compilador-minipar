...
class SemanticAnalyzer:
    ...
    def visit_AssignStmt(self, node: AssignStmt):
        if isinstance(node.alvo, str):
            ...
            self.visit(node.alvo.obj)
            self.visit(node.alvo.indice)
            self.visit(node.expressao)
    ...
    def visit_FunctionCall(self, node: FunctionCall):
        ...
        if sym is None:
            nome = node.nome
            if nome.startswith('math_'):
                for arg in node.argumentos:
                    self.visit(arg)
                return 'number'
            if nome.startswith('turtle_') or nome in ('s_channel', 'c_channel'):
                for arg in node.argumentos:
                    self.visit(arg)
                return 'any'
    ...
    def visit_BinaryExpr(self, node: BinaryExpr):
        ...
        op = node.operador
        DIN = ('any', 'unknown', 'list', 'dict')
        if op in ('+', '-', '*', '/', '%'):
            if op == '+' and 'string' in (t_esq, t_dir):
                return 'string'
            if op == '+' and 'list' in (t_esq, t_dir):
                return 'list'
            if t_esq == 'number' and t_dir == 'number':
                return 'number'
            if t_esq in DIN or t_dir in DIN:
                return 'any'
            self._error(
    ...
    def visit_ParBlock(self, node: ParBlock):
        for stmt in node.statements:
            self.table.enter_scope('par_ramo')
            self.visit(stmt)
            self.table.exit_scope()

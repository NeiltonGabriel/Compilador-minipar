...
class GCCBackend:
    ...
    def _build_command(
        ...
        # Distribuído: usamos um serializador próprio + sockets POSIX (na libc),
        # portanto NÃO há dependência de libcjson. Threads remotas usam pthread.
        if 'distributed' in features or 'network' in features:
            if '-lpthread' not in cmd:
                cmd.append('-lpthread')
